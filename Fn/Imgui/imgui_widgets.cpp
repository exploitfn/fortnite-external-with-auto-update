...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 3.f, label_size.y + style.FramePadding.y * 1.f)); // We want a square shape to we use Y twice
    ...
    auto checkedTopColor = ImColor(89, 3, 247, 255);
    auto checkedBottomColor = ImColor(89, 3, 247, 255);
